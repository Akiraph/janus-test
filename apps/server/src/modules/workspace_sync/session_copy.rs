...
pub fn session_managed_dir(session_id: SessionId) -> String {
...
pub fn session_repo_abs(data_root: &Path, session_id: SessionId) -> PathBuf {
    absoluteish(
        data_root
            .join("workspaces")
            .join("sessions")
...
pub fn remove_session_tree(data_root: &Path, session_id: SessionId) -> anyhow::Result<()> {
