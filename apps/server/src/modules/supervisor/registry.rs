...
//! - runtime: only when a Runtime interface is bound (job/service/delegate_cli).
...
//! when the model replies without tool calls (the supervisor already handles
//! that path).
...
/// All tool specs, regardless of capability. Equivalent to exposing every
/// capability set at once. Kept for tests and external callers that want the
/// full catalogue; turns are fed via `available_tools` instead.
#[allow(dead_code)]
pub fn registry() -> Vec<ToolSpecEntry> {
    full_registry().into_iter().map(|(t, _)| t).collect()
}

/// Tools exposed to the model for a given turn, based on which capabilities
/// are present. `has_runtime` gates bash/job/service/delegate_cli;
/// `has_attachments` gates the attachment.* set.
pub fn available_tools(has_runtime: bool, has_attachments: bool) -> Vec<ToolSpecEntry> {
    full_registry()
        .into_iter()
        .filter(|(_, set)| match set {
            ToolSet::Core => true,
            ToolSet::Runtime => has_runtime,
