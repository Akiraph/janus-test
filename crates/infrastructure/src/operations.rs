...
impl OperationInterface {
    ...
    /// Mark a step succeeded.
    ///
    /// `external_ref` lets a retried handler discover an external result that
    /// side effect that finished after the lease expired - timeout != "did not happen".
