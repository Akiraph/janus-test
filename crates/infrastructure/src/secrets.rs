//! Encrypted deployment secrets and non-reversible secret helpers.
//!
//! The storage format is deliberately independent of any business capability:
//! ciphertext is bound to caller-supplied associated data, while token hashes
//! and fingerprints are derived with separate purpose labels.
...
impl SecretCipher {
    /// Load the deployment key, creating the development key when allowed.
    pub fn load(data_root: &Path, production: bool) -> anyhow::Result<Self> {
